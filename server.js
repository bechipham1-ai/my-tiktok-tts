...
io.on('connection', (socket) => {
    ...
    socket.on('set-username', (username) => {
        ...
        tiktok.connect().then(() => socket.emit('status', '✅ Kết nối thành công')).catch(e => socket.emit('status', '❌ Lỗi kết nối'));
        ...
        // Kích hoạt sự kiện PK (Phải có dòng này)
        tiktok.on('linkMicArmies', () => {
            socket.emit('linkMicArmies');
        });
    });
});
